        ...
        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
          ...
          {item.initiativeName && (
            <span className="text-[11px] text-gray-500 truncate">
              in <span className={dark ? 'text-gray-300' : 'text-gray-700'}>{item.initiativeName}</span>
            </span>
          )}
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search decisions, tickets, commits, notes…"
      ...
      {!query && !loading && (
        <div className={`border-2 border-dashed rounded-2xl py-20 text-center ${dark ? 'border-white/[0.07] text-gray-500' : 'border-gray-200 text-gray-400'}`}>
          ...
          <p className="text-[14px] font-medium">Start typing to search across all initiatives</p>
          <p className={`text-[12px] mt-1 ${muted}`}>Decisions · Sources · Events · Tickets · Commits</p>
        </div>
      )}
      ...
      {query && !loading && !error && results && !hasResults && (
        <div className={`border-2 border-dashed rounded-2xl py-16 text-center ${dark ? 'border-white/[0.07] text-gray-500' : 'border-gray-200 text-gray-400'}`}>
          ...
          <p className={`text-[12px] mt-1 ${muted}`}>Try a different keyword or check your sources have been synced.</p>
        </div>
      )}
      ...
      {hasResults && (
        <div className="space-y-5">
          <p className={`text-[12px] ${muted}`}>{total} result{total !== 1 ? 's' : ''} for <span className="font-semibold">"{query}"</span></p>
            ...
            <section>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className={`text-[11.5px] font-bold uppercase tracking-wide ${muted}`}>
                  Events & Decisions ({results.totalEvents})
                </h3>
                {results.totalEvents > results.events.length && (
                  <span className={`text-[11px] ml-auto flex items-center gap-0.5 ${muted}`}>
                    showing {results.events.length} <ArrowRight size={10} />
                  </span>
                )}
              </div>
              <div className="space-y-2">
                {results.events.map(item => (
                  <ResultCard key={item.id} item={item} dark={dark} />
                ))}
              </div>
            </section>
          )}
          ...
          {results.sources?.length > 0 && (
            <section>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className={`text-[11.5px] font-bold uppercase tracking-wide ${muted}`}>
                  Sources & Documents ({results.totalSources})
                </h3>
                {results.totalSources > results.sources.length && (
                  <span className={`text-[11px] ml-auto flex items-center gap-0.5 ${muted}`}>
                    showing {results.sources.length} <ArrowRight size={10} />
                  </span>
                )}
              </div>
              <div className="space-y-2">
